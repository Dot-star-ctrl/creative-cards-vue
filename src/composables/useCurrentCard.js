...
export default function useCurrentCard() {
    ...
    [selectedCard.value] = allCards.value.filter((card) => card.id === Number(route.params.id));
    ...
    function updateSectionOrder(sectionIndex, direction) {
        ...
        if(selectedCard.value.pages[getPageIndex()].sections[sectionIndex + direction] === undefined) return
