    ...
    setup() {
        ...
        async function uploadFile(event) {
            ...
            reader.onerror = function() {
                state.message = 'There was a problem uploading your message, please try again.'
            }
            reader.readAsDataURL(event.target.files[0]);
            try {
               state.formData = new FormData();
               state.formData.append('upload_preset', 'vqb3uces');
               state.formData.append('file', event.target.files[0]);

               const imageReqData = {
                   url: 'https://api.cloudinary.com/v1_1/di9jl88pb/upload',
                   method: 'POST',
                   data: state.formData
               };
